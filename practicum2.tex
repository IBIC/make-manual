...
use the program in that directory (\texttt{makelinks}) to create all the subject links for visit1.  The current directory won't be in your PATH, so make sure to call it with \texttt{./makelinks}.
...
You can specify to make how many CPUs to use. For example, if we specify the \texttt{-j} flag with an argument of \texttt{2} (processors), we can parallelize execution of \maken{} over two CPUs. 
...
%The FreeSurfer example is documented most recently in \cite{reuter:long12}.
